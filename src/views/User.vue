<template>
  <v-container>
    <v-row justify="space-around">
      <v-card width="400">
        <v-img height="200px" src="https://i.pinimg.com/originals/b8/f2/76/b8f276405f1dd323bfef01abf7232268.jpg">
          ...
          <v-card-title class="white--text mt-8">
            ...
            <p class="ml-3">{{ tienda }}</p>
          </v-card-title>
        </v-img>
        <v-card-text>
          <div class="font-weight-bold ml-8 mb-2"></div>
          <v-form ref="form" v-model="valid" lazy-validation>
            <v-text-field v-model="nombre" :rules="nombreRules" :counter="25" :maxlength="25" label="Nombre" required></v-text-field>
            <v-text-field v-model="email" :rules="emailRules" :counter="50" label="Email" required></v-text-field>
            <v-text-field v-model="tienda" :rules="tiendaRules" :counter="50" label="Nombre de la tienda" required></v-text-field>
          </v-form>
...
export default {
  data: () => ({
    ...
    email: "",
    emailRules: [(v) => !!v || "El email es requerida!", (v) => (v && v.length <= 50) || "El email no puede contener mas de 50 caracteres"],
    tienda: "",
    tiendaRules: [
